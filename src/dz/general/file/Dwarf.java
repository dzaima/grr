...
public class Dwarf {
  ...
  public static class DebugInfo extends DwarfSection {
    ...
    Strtab tab;
    public void finish() {
      tab = strtab(syms.map(c->c.name));
    }
    
    public DwarfSection strSection() {
      return new DwarfStrSection() {
        protected byte[] computeData() { return tab.data(); }
        public String name() { return ".debug_str"; }
      };
    }
    
    public DwarfSection abbrevSection() {
      return new DwarfSection() {
    ...
    public Vec<DwarfSection> allSections() {
      return Vec.of(this, strSection(), abbrevSection());
    }
  }
  
  
  
  public static class DwarfLineInfo extends DwarfSection {
    ...
    public void finish() {
      ...
      // for (int i = 0; i < 1000; i++) strs.add(i+"");
      strt = strtab(strs);
    }
    
    public DwarfSection strSection() {
      return new DwarfStrSection() {
        protected byte[] computeData() { return strt.data(); }
        public String name() { return ".debug_line_str"; }
      };
    }
    
    public Vec<DwarfSection> allSections() {
      return Vec.of(this, strSection());
    }
    ...
    protected byte[] computeData() {
      ...
      w.wS(strt.offs[0]); // ⊑tab.offs
      
      w.w1(1); // file_name_entry_format_count
      w.wULEB128(1); // DW_LNCT_path
      w.wULEB128(31); // DW_FORM_line_strp
      
      w.wULEB128(strt.uoffs.length); // file_names_count
      for (long c : strt.uoffs) w.wS(c);
